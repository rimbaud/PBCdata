#!/Users/rimbaud/Documents/PythonLib/
# -*- coding: utf-8 -*- 
# file: weblist3.py
#Author: Shen Cong
#这是针对一组网页地址的批量操作测试
#这个版本比bs.weblist2.py的改变，是使用循环的方法对列表中的每一个值进行处理
#主要就是方便了R中的数据处理，效率上应该是比原来方法差的.Test to be OK! (^^
	...
	for i in xrange(len(table_layer)):
		...
		if len(ncol)==5:
			...
			#print tbody_tr[i].get_text()
				...
				data=[]
				for item in td_lay:
					data.append(item.get_text().encode('utf-8').replace("\n",""))
				
				supplement = data.pop(-2)
				data.append(operateType)
				data.append(date)
