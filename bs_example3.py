#!/Users/rimbaud/Document/PythonLib
# -*- coding: utf-8 -*- 
# filename: bs_example3.py
#Author: Shen Cong
#这是针对单个网页的测试程序
...
'''
	...
	不足之处是：2006-2009年之间的央行票据数据储存时存在bug。
	那个时段的数据是“3个月,（91天)”这样的形式，
	其他时段的数据是“3个月”这样的形式，这个问题可以在导入R中之后解决
...
#2004
#url = "http://www.pbc.gov.cn/publish/zhengcehuobisi/617/1212/12127/12127_.html"
#url = "http://www.pbc.gov.cn/publish/zhengcehuobisi/617/1211/12113/12113_.html"


#异常值诊断
url = "http://www.pbc.gov.cn/publish/zhengcehuobisi/617/2010/20100510142842724386624/20100510142842724386624_.html"


url ="http://www.pbc.gov.cn/publish/zhengcehuobisi/617/2013/20130926163535768413175/20130926163535768413175_.html"
...
#print "总共有几层table_layer",len(table_layer)
	...
	if len(ncol)==5:
		...
		if str(ncol[0].get_text().encode('utf-8')).find('名称')>0:
			if str(ncol[1].get_text().encode('utf-8')).find('续')>0:
				...
				for i in xrange(1,len(tbody_tr)):
					#print tbody_tr[i].get_text().encode('utf-8')
					#print type(tbody_tr[i].get_text().encode('utf-8'))
					td_lay = tbody_tr[i].find_all('td')
					#print 'length of td',len(td_lay)
					flow=[]
					for item in td_lay:
						#print x.get_text()
						#item.get_text.encode('utf-8').replace("\n","") 
						flow.append(item.get_text().encode('utf-8').replace("\n",""))
					...
					#data = str(tbody_tr[i].get_text().encode('utf-8')).split() # 这是删除字符串中的空白字符串的方法 replace('\n'," ")
					#data.append(operateType)  #把新信息添加进入原来的数组List
					#print type(tbody_tr[i].get_text().encode('utf-8')) ##it's str
					#将处理完的数据写入txt文件保存
					f = csv.writer(open('class2.txt','a+b'))  # “+”号表示追加写
					#f.writerow(['term','amount','rate','type','date'])
					f.writerow(flow)
					#print data
					#print '\xe5\xa4\xae\xe8\xa1\x8c\xe7\xa5\xa8\xe6\x8d\xae\xef\xbc\x88\xe7\xbb\xad\xef\xbc\x89'.decode('utf-8') ##it works OK
...
#f= csv.writer(open("class1.txt", "w"))
#f.writerow(["HouseCandidate", "votes"])
